...
import {
  mapRankingMoreHoursAPIResponse,
  mapRankingIncreaseHoursAPIResponse,
  mapAverageAPIResponse,
} from "../dto/mapDTO.js";
...
export const getRankingIncreasedHours = async () => {
  ...
  const [ranking] = await db.query(query);
  return ranking.map(mapRankingIncreaseHoursAPIResponse);
};
...
export const getRankingAboveAverage = async () => {
  ...
  return average.map(mapAverageAPIResponse);
};
