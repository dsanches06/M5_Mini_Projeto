...
export const getTimeLogs = async (req, res) => {
  try {
    ...
    res.status(500).json({ error: `Error fetching time logs: ${error.message}` });
  }
};
...
export const createTimeLog = async (req, res) => {
  try {
    ...
    if (!task_id || !user_id || !hours) {
      return res.status(400).json({ error: "task_id, user_id, and hours are required" });
    }
    const timeLog = await timeLogService.createTimeLog(req.body);
    res.status(201).json(timeLog);
  } catch (error) {
    res.status(400).json({ error: `Error creating time log: ${error.message}` });
  }
};
...
export const updateTimeLog = async (req, res) => {
  try {
    ...
    res.status(400).json({ error: `Error updating time log: ${error.message}` });
  }
};
...
export const deleteTimeLog = async (req, res) => {
  try {
    ...
    res.status(400).json({ error: `Error deleting time log: ${error.message}` });
  }
};
