...
export function processTask(task) {
    ...
    const user = ((_a = task.getAssignees) === null || _a === void 0 ? void 0 : _a.call(task)[0]) ? undefined : undefined;
    const currentStatus = task.getStatus();
    const nextStatus = StateTransitions.getNextStatus(currentStatus);
    if (type === "Bugs" && !user) {
        SystemLogger.log(`ERRO: Bug ${task.getTitle()} (id:${task.getId()}) não possui usuário atribuído e não pode ser processado.`);
        return;
    }
    ...
    switch (type) {
        case "Bugs":
            ...
            if (!GlobalValidators.isValidTitle(title)) {
                SystemLogger.log(`ERRO: Bug ${task.getId()} tem título inválido. O título precisa ter pelo menos 10 caracteres.`);
            ...
            if (!user) {
                SystemLogger.log(`ERRO: Bug ${task.getId()} não possui usuário atribuído e regras mais rígidas impedem o processamento.`);
                // TODO: Implementar notificação assincróna
                // await NotificationService.createNotification({
        ...
        case "Feature":
            ...
            if (!GlobalValidators.isNonEmpty(title)) {
                SystemLogger.log(`ERRO: Feature ${task.getId()} possui título vazio e não pode ser processado.`);
                // TODO: Implementar notificação assincróna
                // if (user) {
                //   await NotificationService.createNotification({
                //     userId: user.getId(),
            ...
            if (!GlobalValidators.minLength(title, 3)) {
                SystemLogger.log(`WARN: Feature ${task.getId()} possui título curto, mas será processado por regras mais flexíveis.`);
            }
            break;
        default:
    ...
    processTransition(task, type, user, currentStatus, nextStatus, notifyOnSuccess, notifyAdminsOnSuccess);
}
// Helper que encapsula o try/catch e logging/notifications para reduzir duplicação
function processTransition(task, type, user, currentStatus, nextStatus, notifyOnSuccess, notifyAdminsOnSuccess = false) {
    try {
        task.moveTo(nextStatus);
        // TODO: Implementar AutomationRulesService.applyRules() via API
        // AutomationRulesService.applyRules(task);
        SystemLogger.log(`INFO: A tarefa ${task.getTitle()} do tipo ${type} atribuido ao ${user === null || user === void 0 ? void 0 : user.getName()} foi processado [${currentStatus.toString()} -> ${nextStatus.toString()}].`);
        // Log automático extra para Bugs
        if (type === "Bugs") {
            SystemLogger.log(`AUTO-LOG: Bug ${task.getId()} processado com regras rígidas em ${new Date().toLocaleString("pt-PT")}.`);
        }
        ...
        SystemLogger.log(`ERRO: Falha ao processar a tarefa ${task.getTitle()} do tipo ${type} atribuido ao ${user === null || user === void 0 ? void 0 : user.getName()}. Motivo: ${error.message}`);
        if (user) {
            // TODO: Implementar notificação assincróna
            // await NotificationService.createNotification({
            //   userId: user.getId(),
